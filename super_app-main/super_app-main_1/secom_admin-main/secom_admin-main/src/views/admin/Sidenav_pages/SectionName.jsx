...
function SectionName() {
    ...
    const validationSchema = Yup.object().shape({
        ...
        background_image_desktop: Yup.mixed().required('Desktop Image is required'),
        background_image_desktop: Yup.mixed().required('Mobile Image is required'),
    ...
    const validationSchemaEdit = Yup.object().shape({
        ...
        url: Yup.string()
            .url('Button URL must be a valid URL')
            .required('Button URL is required'),
        background_image_desktop: Yup.mixed().notRequired(),
        background_image_desktop: Yup.mixed().notRequired(),
        date: Yup.string().required('Date is required'),
        time_period: Yup.string().required('Time is required'),
    ...
    const { reset, control, handleSubmit, setValue, formState: { errors } } = useForm({
        resolver: yupResolver(openAddModal ? validationSchema : validationSchemaEdit), // Dynamic validation schema based on the modal state
    });
    ...
    const handleEditRow = (section) => {
        ...
        setValue('product_variation_id', section.product_variation_id);
        setValue('background_image_desktop', section.background_image_desktop);
        setValue('background_image_desktop', section.background_image_desktop);
        setValue('button_name', section.button_name);
        setValue('url', section.url);
        ...
        const fetchData = async () => {
            try {
                const [brandResponse, categoryResponse, subCategoryResponse, productResponse, sessionResponse] = await Promise.all([
                    ...
                    axios.get('https://yrpitsolutions.com/ecom_backend/api/admin/get_all_product_variation'),
                ...
                setSessionNames(sessionResponse.data);
    ...
    const handleFormSubmit = async () => {
        ...
        const formData = {
            section_name_id: selectedSession,
            // type: selectedType,
            button_name: buttonName,
            url: buttonUrl,
            background_image_desktop: desktopImage,
            background_image_mobile: mobileImage,
            date: selectedDate,
            time_period: selectedTime,
        };

        if (selectedType === 'brand') {
            formData.brand_id = selectedBrand;
        } else if (selectedType === 'category') {
            formData.category_id = selectedCategory;
        } else if (selectedType === 'subcategory') {
            formData.subcategory_id = selectedSubCategory;
        } else if (selectedType === 'productvariation') {
            formData.product_variation_id = selectedProducts;
        }
    ...
    const handleFormUpdate = async () => {
        try {
            ...
            formData.append('url', buttonUrl);
            formData.append('background_image_desktop', desktopImage); // Assuming desktopImage is a file input
            formData.append('background_image_desktop', mobileImage); // Assuming mobileImage is a file input
            ...
            if (selectedType === 'productvariation') {
                selectedProducts.forEach((productId) => {
                    formData.append('product_variation_id[]', productId);
                });
            }
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    {validationErrors.section_name_id && (
                                        <p className="text-red-500 text-sm mt-1">{validationErrors.section_name_id}</p>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="productvariation">Product Variation</option>
                                    </select>
                            ...
                            {/* Conditional Dropdowns for Type */}
                            {selectedType === "productvariation" && (
                                <div className="mb-6 relative">
                                    <label className="block text-lg text-gray-600 font-medium mb-2">
                                        Select Product Variation<span className="text-red-500">*</span>
                                    </label>
                                        ...
                                        <div className="absolute z-10 w-full mt-2 border border-gray-300 bg-white rounded-md shadow-lg">
                                            <div className="p-2">
                                                ...
                                                <div className="max-h-40 overflow-y-auto">
                                                    ...
                                                    {products
                                                        .filter((product) => {
                                                            const matchesSearch = product?.variation_name?.toLowerCase()?.includes(searchQuery?.toLowerCase());
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {selectedProducts.map((productId) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {productItem ? productItem.variation_name : ""}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-800 focus:outline-none"
                                    />
                                    {validationErrors.button_name && (
                                        <p className="text-red-500 text-sm mt-1">{validationErrors.button_name}</p>
                                    )}
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-800 focus:outline-none"
                                    />
                                    {validationErrors.background_image_desktop && (
                                        <p className="text-red-500 text-sm mt-1">{validationErrors.background_image_desktop}</p>
                                    )}
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    {validationErrors.time_period && (
                                        <p className="text-red-500 text-sm mt-1">{validationErrors.time_period}</p>
                                    )}
                                </div>
                            </div>
