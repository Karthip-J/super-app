        ...
        for (const p of partners) {
            try {
                ...
                if (!user) {
                    ...
                    console.log(`  Created User for ${p.fullName}: ${user._id}`);
                } else {
                ...
                if (!sp) {
                    ...
                    console.log(`  Created ServicePartner for ${p.fullName}: ${sp._id}`);
                } else {
                    // Update only if documents changed or missing
                    if (!sp.verificationDocuments || sp.verificationDocuments.length === 0) {
                        sp.verificationDocuments = documents;
                        sp.status = 'active';
                        await sp.save();
                        console.log(`  Updated ServicePartner documents for ${p.fullName}`);
                    }
                }
