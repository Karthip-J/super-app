...
const run = async () => {
    try {
        ...
        const collection = mongoose.connection.collection('servicepartners');
        const indexes = await collection.indexes();
        console.log('Current SP Indexes:', indexes.map(i => i.name));

        const toDrop = [
            'personal_info.email_1',
            'application_id_1',
            'partner_code_1',
            'personal_info.phone_1',
            'business_info.business_type_1'
        ];
