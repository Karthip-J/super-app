...
function EditAddressValues() {
    ...
    useEffect(() => {
        if (address) {
            setFormData({
                ...
                customLabel: address.selectedAddressType && !buttons.includes(address.selectedAddressType)
                    ? address.selectedAddressType
                    : ''
            });
            if (address.selectedAddressType) {
                if (buttons.includes(address.selectedAddressType)) {
    ...
    const handleSave = () => {
        ...
        if (typeof index === 'number') {
            storedAddresses[index] = updatedAddress;
        } else {
            storedAddresses.push(updatedAddress);
        }
        localStorage.setItem(CLOTHES_ADDRESS_KEY, JSON.stringify(storedAddresses));
    ...
    return (
        <div className='bg-[#FDFDFD] min-h-screen font-sans'>
            <ClothesHeader />
            <div className="px-4 pt-28 space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-black text-gray-900 leading-tight">Edit delivery address</h2>
                        <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider font-bold">Update your delivery details</p>
                    </div>
